#!/usr/bin/env python3
"""
16_train_prior_corrected_wsf_diffnorm.py

Baseline 2d: prior-corrected dilated residual WSF differentiable mass allocator.
...
Model
-----
Dilated residual fully convolutional network with an unconstrained logit output.
The logits correct a fixed WSF-derived prior in log-score space.

Loss
----
The CNN predicts raw logits from random initialization. A positive prior-corrected
score surface is constructed as:

score = prior ** prior_power * exp(logits / prior_temperature)

Scores are differentiably normalized within each coarse cell and multiplied by
that cell's GHSL target. Training uses the resulting mass-preserving fine
prediction:
...
- cap_loss optionally discourages predictions above a physical per-pixel maximum
...
Example
-------
python 16_train_prior_corrected_wsf_diffnorm.py \
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train prior-corrected dilated residual WSF diffnorm allocator.")
    ...
    p.add_argument("--prior-out", default=None, help="Optional output raster path for the constructed WSF prior")
    ...
    p.add_argument(
        "--prior-mode",
        default="wsf_blend",
        choices=["none", "wsf_bin", "wsf_density", "wsf_blend", "wsf_detail"],
        help="WSF prior used to shape within-cell allocation before differentiable normalization.",
    )
    p.add_argument(
        "--prior-density-band",
        type=int,
        default=2,
        help="1-based WSF feature band used as the primary density prior, usually wsf_dens_5.",
    )
    p.add_argument(
        "--prior-density2-band",
        type=int,
        default=3,
        help="1-based WSF feature band used as the secondary density prior, usually wsf_dens_11.",
    )
    p.add_argument(
        "--prior-blend-weight",
        type=float,
        default=0.5,
        help="Weight on prior-density-band when prior-mode=wsf_blend. The secondary band receives 1 - weight.",
    )
    p.add_argument("--prior-power", type=float, default=1.0, help="Exponent alpha applied to the WSF prior.")
    p.add_argument(
        "--prior-temperature",
        type=float,
        default=1.0,
        help="Temperature tau for learned log-score correction. Larger values weaken the learned correction.",
    )
    p.add_argument(
        "--prior-floor",
        type=float,
        default=1e-3,
        help="Minimum positive prior value to avoid zero denominators and allow small allocation outside strong WSF-density pixels.",
    )
    p.add_argument("--prior-eps", type=float, default=1e-6, help="Numerical epsilon used in prior construction.")
    p.add_argument(
        "--detail-sigma",
        type=float,
        default=4.0,
        help="Gaussian blur sigma, in pixels, used to construct WSF detail ratio for prior-mode=wsf_detail.",
    )
    p.add_argument("--detail-gamma", type=float, default=0.5, help="Strength of WSF detail ratio when prior-mode=wsf_detail.")
    p.add_argument("--detail-clip-min", type=float, default=0.25, help="Minimum clipping value for WSF detail ratio.")
    p.add_argument("--detail-clip-max", type=float, default=4.0, help="Maximum clipping value for WSF detail ratio.")
    p.add_argument(
        "--cap-weight",
        type=float,
        default=0.0,
        help="Optional penalty weight for predictions exceeding cap-value. Set >0 to discourage values above physical maximum.",
    )
    p.add_argument(
        "--cap-value",
        type=float,
        default=100.0,
        help="Physical maximum built-up surface per 10m pixel. Default is 100 square meters.",
    )
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.prior_power < 0:
        raise ValueError("--prior-power must be >= 0")
    if args.prior_temperature <= 0:
        raise ValueError("--prior-temperature must be > 0")
    if not (0.0 <= args.prior_blend_weight <= 1.0):
        raise ValueError("--prior-blend-weight must be between 0 and 1")
    if not (0.0 < args.prior_floor < 1.0):
        raise ValueError("--prior-floor must be > 0 and < 1")
    if args.prior_eps <= 0:
        raise ValueError("--prior-eps must be > 0")
    if args.prior_mode == "wsf_detail" and args.detail_sigma <= 0:
        raise ValueError("--detail-sigma must be > 0 when --prior-mode=wsf_detail")
    if args.detail_clip_min <= 0:
        raise ValueError("--detail-clip-min must be > 0")
    if args.detail_clip_max <= args.detail_clip_min:
        raise ValueError("--detail-clip-max must be greater than --detail-clip-min")
    if args.cap_value <= 0:
...
def robust_unit_scale(arr: np.ndarray, valid_mask: np.ndarray, eps: float = 1e-6) -> np.ndarray:
    """Return a 0..1 version of arr using robust valid-pixel scaling."""
    ...
    if vmax <= 1.5:
        ...
        q_hi = float(np.nanpercentile(vals, 99.0))
        if q_hi <= eps:
            q_hi = vmax if vmax > eps else 1.0
        scaled = arr.astype(np.float32, copy=True) / q_hi
...
def require_band(arr: np.ndarray, band_1based: int, label: str) -> np.ndarray:
    if not (1 <= band_1based <= arr.shape[0]):
        raise ValueError(f"{label} must be between 1 and {arr.shape[0]}")
    return arr[band_1based - 1]


def build_wsf_prior(
    wsf_feats: np.ndarray,
    wsf_bin: np.ndarray,
    valid_mask: np.ndarray,
    mode: str,
    wsf_band_1based: int,
    density_band_1based: int,
    density2_band_1based: int,
    blend_weight: float,
    floor: float,
    eps: float,
    detail_sigma: float,
    detail_gamma: float,
    detail_clip_min: float,
    detail_clip_max: float,
) -> np.ndarray:
    """Construct positive WSF prior q with shape (H, W)."""
    ...
    if mode == "wsf_bin":
        support = np.where(np.isfinite(wsf_bin) & (wsf_bin > 0), 1.0, floor).astype(np.float32)
        q[valid_mask] = support[valid_mask]
        return q

    dens1 = robust_unit_scale(
        require_band(wsf_feats, density_band_1based, "--prior-density-band"),
        valid_mask,
        eps=eps,
    )
    if mode == "wsf_density":
        q = floor + (1.0 - floor) * dens1
        q[~valid_mask] = 0.0
        return q.astype(np.float32, copy=False)

    dens2 = robust_unit_scale(
        require_band(wsf_feats, density2_band_1based, "--prior-density2-band"),
        valid_mask,
        eps=eps,
    )
    base = blend_weight * dens1 + (1.0 - blend_weight) * dens2
    base = np.clip(base, 0.0, 1.0)
    base_pos = floor + (1.0 - floor) * base

    if mode == "wsf_blend":
        base_pos[~valid_mask] = 0.0
        return base_pos.astype(np.float32, copy=False)

    if mode == "wsf_detail":
        try:
            from scipy.ndimage import gaussian_filter
        except ImportError as exc:
            raise RuntimeError("--prior-mode=wsf_detail requires scipy.ndimage.gaussian_filter") from exc

        smooth = gaussian_filter(base_pos, sigma=detail_sigma)
        detail = base_pos / (smooth + eps)
        detail = np.clip(detail, detail_clip_min, detail_clip_max)
        q = base_pos * (detail ** detail_gamma)
...
class DilatedResidualLogitNet(nn.Module):
    def __init__(
        self,
        in_channels: int,
        hidden: int = 32,
...
def make_prior_corrected_score(
    logits: torch.Tensor,
    prior: torch.Tensor,
    ...
    temperature: float,
    score_floor: float,
    log_score_clip: float = 20.0,
) -> torch.Tensor:
    """Create positive score surface from fixed prior and learned correction."""
    log_prior = torch.log(prior.clamp_min(score_floor))
    log_score = float(prior_power) * log_prior + logits / float(temperature)
    log_score = torch.clamp(log_score, -log_score_clip, log_score_clip)
    score = torch.exp(log_score) * valid_mask
    score = torch.clamp(score, min=score_floor) * valid_mask
    return score


def cap_loss_2d(y: torch.Tensor, valid_mask: torch.Tensor, cap_value: float) -> torch.Tensor:
    excess = torch.relu(y - float(cap_value))
    denom = valid_mask.sum() + 1e-8
    return ((excess**2) * valid_mask).sum() / denom / (float(cap_value) ** 2)
...
def plot_losses(history: List[dict], out_path: Path) -> None:
    ...
    cap = [h["cap_loss"] for h in history]
    ...
    plt.plot(epochs, cap, label="cap loss")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
...
def main() -> None:
    ...
    prior_np = build_wsf_prior(
        wsf_feats=wsf_feats,
        wsf_bin=wsf_bin,
        valid_mask=allocation_mask,
        mode=args.prior_mode,
        wsf_band_1based=args.wsf_band,
        density_band_1based=args.prior_density_band,
        density2_band_1based=args.prior_density2_band,
        blend_weight=args.prior_blend_weight,
        floor=args.prior_floor,
        eps=args.prior_eps,
        detail_sigma=args.detail_sigma,
        detail_gamma=args.detail_gamma,
        detail_clip_min=args.detail_clip_min,
        detail_clip_max=args.detail_clip_max,
    )
    prior_for_output = prior_np.copy()
    prior_for_output[~allocation_mask] = np.nan
    ...
    prior_t = torch.from_numpy(prior_np[None, None, ...]).to(device=device, dtype=torch.float32)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        logits = model(x_t)
        score = make_prior_corrected_score(
            logits=logits,
            prior=prior_t,
            valid_mask=valid_t,
        ...
        cap_loss = cap_loss_2d(pred_mass, valid_t, args.cap_value)
    ...
    with torch.no_grad():
        ...
        final_cap = cap_loss_2d(pred_mass, valid_t, args.cap_value)
    ...
    metrics["final_cap_loss"] = float(final_cap.detach().cpu().item())
    ...
    valid_prior = prior_np[allocation_mask]
    prior_diagnostics = {
        "min": float(np.nanmin(valid_prior)),
        "p01": float(np.nanpercentile(valid_prior, 1)),
        "p05": float(np.nanpercentile(valid_prior, 5)),
        "median": float(np.nanmedian(valid_prior)),
        "p95": float(np.nanpercentile(valid_prior, 95)),
        "p99": float(np.nanpercentile(valid_prior, 99)),
        "max": float(np.nanmax(valid_prior)),
    }
    ...
    prior_out = Path(args.prior_out).expanduser().resolve() if args.prior_out else None

    write_raster(pred_out, pred_np, profile)
    print(f"[INFO] Saved raw prior-corrected score raster to: {pred_out}")
    ...
    if prior_out is not None:
        write_raster(prior_out, prior_for_output, profile)
        print(f"[INFO] Saved WSF prior raster to: {prior_out}")
    ...
    report = {
        ...
        "prior_density_band": int(args.prior_density_band),
        "prior_density2_band": int(args.prior_density2_band),
        ...
        "detail_clip_min": float(args.detail_clip_min),
        "detail_clip_max": float(args.detail_clip_max),
        "cap_weight": float(args.cap_weight),
        "cap_value": float(args.cap_value),
        "prior_out": str(prior_out) if prior_out is not None else None,
        ...
        "output_descriptions": {
            "pred_out": "raw positive prior-corrected score surface for debugging; not mass-preserving",
            "pred_norm_out": "exact CPU-renormalized mass-preserving fine prediction",
            "prior_out": "constructed WSF prior q; optional diagnostic raster",
        },
        "metric_groups": {
