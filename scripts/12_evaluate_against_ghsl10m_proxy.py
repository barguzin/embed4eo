#!/usr/bin/env python3
"""
Quantitative evaluation of downscaled 2019 predictions against an external
GHSL 10 m reference proxy.
...
training label or exact same-year ground truth. The default wording assumes
2019 predictions are compared to the separate GHSL 2018 10 m product. Metrics
are reported at the native fine grid and at coarser block-aggregation scales.

Example
-------
python 12_evaluate_against_ghsl10m_proxy.py \
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate downscaled built-up predictions against an external GHSL 10 m proxy reference."
    )
    p.add_argument(
        "--predictions",
        nargs="+",
    ...
    p.add_argument(
        ...
        type=int,
        default=2019,
        help="Prediction year used for run metadata and figure labels. Default: 2019.",
    )
    p.add_argument(
        "--reference-year",
        type=int,
        default=2018,
        help="GHSL 10 m reference year used for metadata and figure labels. Default: 2018.",
    )
    p.add_argument(
        "--wsf",
        default=None,
    ...
    p.add_argument(
        ...
        default=1e-6,
        help=(
            "Minimum positive reference value included in MAPE denominator. "
            "Reference values <= this are excluded from MAPE. Default: 1e-6."
        ),
    )
    p.add_argument(
        "--min-valid-fraction",
...
def scalar_metrics(
    ...
    abs_err = np.abs(err)
    mape_mask = ref > mape_epsilon
    sse = float(np.sum(err ** 2))
    sst = float(np.sum((ref - np.mean(ref)) ** 2))
    ...
    out.update(
        {
            ...
            "mape": float(np.mean(abs_err[mape_mask] / np.abs(ref[mape_mask])) * 100.0) if np.any(mape_mask) else np.nan,
            "mape_ref_positive_pixels": float(np.sum(mape_mask)),
            "mape_excluded_pixels": float(np.sum(~mape_mask)),
...
def write_summary_figure(rows: List[dict], out_path: Path, prediction_year: int, reference_year: int) -> None:
    ...
    pseudo_r2_rows = metric_rows("pseudo_r2")
    top_rows = [
        r for r in rows
    ...
    fig.suptitle(f"GHSL 10 m proxy validation: predictions {prediction_year} vs reference {reference_year}")
    ...
    axes[0, 1].set_title("RMSE by aggregation scale")
    ...
    for model, g in group_by_model(pseudo_r2_rows).items():
        x = [float(row["scale_m"] if row["scale_m"] is not None else row["scale_factor"]) for row in g]
        y = [float(row["value"]) for row in g]
        axes[1, 1].plot(x, y, marker="o", label=model)
    axes[1, 1].axhline(0, color="black", linewidth=0.8, linestyle="--")
    axes[1, 1].set_title("Pseudo-R² by aggregation scale")
    axes[1, 1].set_xlabel("Scale")
    axes[1, 1].set_ylabel("1 - SSE/SST")
    axes[1, 1].legend()
...
def main() -> None:
    ...
    if args.output_json:
        ...
        payload = {
            "note": (
                f"Reference is treated as an external high-resolution proxy, not direct ground truth "
                f"or a training label. Default interpretation: {args.prediction_year} predictions "
                f"compared to the separate GHSL {args.reference_year} 10 m product."
            ),
            ...
            "mape_epsilon": float(args.mape_epsilon),
            "prediction_year": int(args.prediction_year),
    ...
    if args.output_fig:
        ...
        write_summary_figure(rows, out_fig, args.prediction_year, args.reference_year)
